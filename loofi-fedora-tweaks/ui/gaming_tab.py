...
class GamingTab(QWidget):
    def __init__(self):
        ...
        # Header
        header = QLabel("Gaming Optimizations")
        ...
        # Feral Gamemode
        self.btn_gamemode = QPushButton("Install Feral GameMode")
        self.btn_gamemode.clicked.connect(self.install_gamemode)
        perf_layout.addWidget(self.btn_gamemode)
        
        self.lbl_gamemode_status = QLabel("GameMode Status: Unknown")
        perf_layout.addWidget(self.lbl_gamemode_status)
        
        # MangoHud
        btn_mangohud = QPushButton("Install MangoHud & Goverlay")
        ...
        # ProtonUp-Qt
        btn_protonup = QPushButton("Install ProtonUp-Qt (Flatpak)")
        btn_protonup.clicked.connect(self.install_protonup)
        steam_layout.addWidget(btn_protonup)
        
        # Steam Devices
        btn_steam_devices = QPushButton("Install Steam Devices (Controller Support)")
    ...
    def install_gamemode(self):
        ...
        QMessageBox.information(self, "Installing", "GameMode installation started. Check Updates tab for progress if you want.")
    ...
    def check_gamemode_status(self):
        ...
        try:
            ...
            if "active" in result.stdout:
                self.lbl_gamemode_status.setText("GameMode Status: ✅ Active (Service Running)")
                self.btn_gamemode.setEnabled(False)
                self.btn_gamemode.setText("GameMode Installed")
            else:
                 # Check if installed but not running
                 res_rpm = subprocess.run(["rpm", "-q", "gamemode"], capture_output=True)
                 if res_rpm.returncode == 0:
                     self.lbl_gamemode_status.setText("GameMode Status: ⚠️ Installed but Inactive")
                     self.btn_gamemode.setText("Reinstall GameMode")
                 else:
                     self.lbl_gamemode_status.setText("GameMode Status: ❌ Not Installed")
        except Exception as e:
            self.lbl_gamemode_status.setText(f"Status check failed: {e}")
    ...
    def install_mangohud(self):
        ...
        QMessageBox.information(self, "Installing", "MangoHud & Goverlay installation started.")

    def install_protonup(self):
        self.runner.run_command("flatpak", ["install", "flathub", "net.davidotek.pupgui2", "-y"])
        QMessageBox.information(self, "Installing", "ProtonUp-Qt installation started.")

    def install_steam_devices(self):
        self.runner.run_command("pkexec", ["dnf", "install", "steam-devices", "-y"])
        QMessageBox.information(self, "Installing", "Steam Devices support installation started.")
